...
['$timeout', 'showErrorsConfig', '$interpolate', ($timeout, showErrorsConfig, $interpolate) ->
    ...
    linkFn = (scope, el, attrs, formCtrl) ->
      ...
      inputEl   = el[0].querySelector '.form-control[name],input[name],select[name],password[name],email[name],datetime[name],datetime-local[name],date[name],month[name],time[name],week[name],number[name],url[name],search[name],tel[name],color[name],textarea[name]'
